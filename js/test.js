...
function bench() {
  ...
  suite.add('go prepared', function() {
    fuzzysort.go('nnnne', testdata_prepared.books)
    fuzzysort.go('e', testdata_prepared.books)
    fuzzysort.go('mrender.h', testdata_prepared.books)
  })
  suite.add('go key', function() {
    fuzzysort.go('nnnne', testdata_obj.books, {key: 'str'})
    fuzzysort.go('e', testdata_obj.books, {key: 'str'})
    fuzzysort.go('mrender.h', testdata_obj.books, {key: 'str'})
  })
  suite.add('go keys', function() {
    fuzzysort.go('nnnne', testdata_obj.books, {keys: ['str']})
    fuzzysort.go('e', testdata_obj.books, {keys: ['str']})
    fuzzysort.go('mrender.h', testdata_obj.books, {keys: ['str']})
  })
  suite.add('go str', function() {
    fuzzysort.go('nnnne', testdata.books)
    fuzzysort.go('e', testdata.books)
    fuzzysort.go('mrender.h', testdata.books)
  })
  ...
  suite.add('goAsync', function(deferred) {
    var count = 0
    fuzzysort.goAsync('e', testdata_prepared.books).then(()=>{count+=1; if(count===3)deferred.resolve()})
    fuzzysort.goAsync('a', testdata_prepared.books).then(()=>{count+=1; if(count===3)deferred.resolve()})
    fuzzysort.goAsync('mrender.h', testdata_prepared.books).then(()=>{count+=1; if(count===3)deferred.resolve()})
  }, {defer:true})

  // suite.add('goAsync.cancel()', function(deferred) {
  //   const p = fuzzysort.goAsync('e', testdata_prepared.books)
